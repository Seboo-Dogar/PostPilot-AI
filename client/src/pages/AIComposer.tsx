...
import { ArrowRightIcon, CalculatorIcon, ClockIcon, HistoryIcon, Loader2Icon, TimerIcon, Wand2Icon, XIcon } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea placeholder="Share your idea... (e.g A post about the launch of our new eco-friendly coffee beans)" value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full px-6 py-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40" />
            <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
              <button onClick={() => setGenerateImage(!generateImage)} className="flex items-center gap-3 bg-red-50 px-3 py-2 rounded-lg">
                ...
                <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${generateImage ? "bg-red-500" : "bg-slate-200"}`}>
                  <span className={`pointer-events-none size-4 transform translate-y-0.5 rounded-full bg-white transition ${generateImage ? "translate-x-4.5" : "translate-x-0.5"}`}></span>
                </div>
              </button>
              <button onClick={handleGenerate} disabled={loading} className="bg-slate-900 hover:bg-slate-800 flex items-center text-white gap-2 px-4 py-2 rounded-lg">
                {loading ? (
                  <>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {tones.map((t) => (
            <button key={t} onClick={() => setTone(t)} className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "bg-red-500 border-red-500 text-white" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300"}`}>
              {t}
            </button>
          ))}
        </div>
...

          {
            generations.length === 0 && (
              <div className="col-span-full text-center py-20 space-y-2">
                <div className="flex items-center justify-center mx-auto size-12 bg-slate-50 text-slate-300">
                  <Wand2Icon className="size-6" />
                </div>
                <p className="text-slate-400 text-sm">No content generated yet. Try to generate some content using the AI.</p>
              </div>
            )
          }
        </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
                ...
                <div className="space-y-6">
                  <div className="">
                    <label className="text-xs block text-slate-600 uppercase tracking-widest mb-4">Select Channels</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="relative">
                      <CalculatorIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                      <input type="date" value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all" />
                    </div>
                    <div className="relative">
                      <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                      <input type="time" value={scheduledTime} onChange={(e) => setScheduledTime(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all" />
                    </div>
                  </div>
                </div>
